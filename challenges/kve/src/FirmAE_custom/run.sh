...
function run_emulation()
{
    ...
    if [ ${OPTION} = "analyze" ]; then
        ...
        # just run mode
        # ================================
        # Queue forwarding setup after the network check. The networking check ensures that the network works and is online.
        (check_network ${IP} false && add_docker_qemu_forwarding ${IID}) &
        ${WORK_DIR}/run.sh
