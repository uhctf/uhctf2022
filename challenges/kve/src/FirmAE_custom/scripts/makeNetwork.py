...
# Copy honeypot stuff from Docker to QEMU
# `cp` if inside the docker, else (pre-emulation outside docker) just create the hook file which should always exist
cp -r /work/custom/* ${WORK_DIR}/image/firmadyne/ || touch ${WORK_DIR}/image/firmadyne/hook.sh
chmod a+x ${WORK_DIR}/image/firmadyne/hook.sh
