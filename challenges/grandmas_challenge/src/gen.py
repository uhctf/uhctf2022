...
def generate(words="words", x=20, y=20):
    n_words = 20
    NMAX = 32
    ...
    def _make_wordsearch(nrows, ncols, wordlist, allow_backwards_words=True,
        ...
        choices = fill_grid_randomly(grid)
        remove_mask(grid)
    ...
    def make_wordsearch(*args, **kwargs):
        ...
        for i in range(NATTEMPTS):
            grid, solution, choices = _make_wordsearch(*args, **kwargs)
            if grid:
    ...
    def show_wordsearch_text(grid, wordlist):
        ...
        out = ""
        out += show_grid_text(grid)
        #print()
        out += "\n"
        out += show_wordlist_text(wordlist)
        return out
    ...
    def get_wordlist(wordlist_filenamem, n):
    ...
    wordlist = sorted(get_wordlist(wordlist_filename, n_words), key=lambda w: len(w),
                      reverse=True)
    ...
    grid, solution, choices = make_wordsearch(nrows, ncols, wordlist, allow_backwards_words,
                                     mask)

    # If we fitted the words to the grid, show it in text format and save SVG files
    # of the grid and its solution.
    if grid:
        show_to_user = show_wordsearch_text(grid, wordlist)
        key = "".join(choices)
        return show_to_user, key
